...
std::vector<Condition> EndConditionSampler::SampleLonEndConditionsForCruising(const double ref_cruise_speed) const {
  ...
  for (const auto& time : time_samples) {
    ...
    size_t num_of_mid_points = std::min(static_cast<size_t>(FLAGS_num_velocity_sample - 2),
...
const std::string& obstacle_id, std::vector<SamplePoint>* const sample_points) const {
  ...
  for (const auto& path_time_point : follow_path_time_points) {
    ...
    double s_gap = FLAGS_default_lon_buffer / static_cast<double>(FLAGS_num_sample_follow_per_timestamp - 1);
...
void EndConditionSampler::QueryOvertakePathTimePoints(const common::VehicleConfig& vehicle_config,
const std::string& obstacle_id, std::vector<SamplePoint>* sample_points) const {
  std::vector<STPoint> overtake_path_time_points = ptr_path_time_graph_->GetObstacleSurroundingPoints(
      obstacle_id,FLAGS_numerical_epsilon, FLAGS_time_min_density);
