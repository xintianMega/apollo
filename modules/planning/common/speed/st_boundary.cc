...
bool STBoundary::GetUnblockSRange(const double curr_time, double* s_upper, double* s_lower) const {
  ...
  *s_lower = 0.0;
  ...
  const double r = (left == right ? 0.0 : (curr_time - upper_points_[left].t()) /
...
bool STBoundary::GetBoundarySlopes(const double curr_time, double* ds_upper, double* ds_lower) const {
  ...
  if (has_prev && has_next) {
    *ds_upper = ((next_s_upper - curr_s_upper) / kTimeIncrement + (curr_s_upper - prev_s_upper) / kTimeIncrement) * 0.5;
    *ds_lower = ((next_s_lower - curr_s_lower) / kTimeIncrement + (curr_s_lower - prev_s_lower) / kTimeIncrement) * 0.5;
    return true;
  }
  if (has_prev) {
...
STBoundary STBoundary::ExpandByT(const double t) const {
  ...
  point_pairs.front().first.set_s(std::fmin(point_pairs.front().second.s() - kMinSEpsilon,point_pairs.front().first.s()));
  ...
  point_pairs.emplace_back(STPoint(lower_points_.back().s() + t * lower_right_delta_s / right_delta_t, lower_points_.back().t()
  + t), STPoint(upper_points_.back().s() + t * upper_right_delta_s / right_delta_t, upper_points_.back().t() + t));
  point_pairs.back().second.set_s(std::fmax(point_pairs.back().second.s(), point_pairs.back().first.s() + kMinSEpsilon));
...
bool STBoundary::IsValid(const std::vector<std::pair<STPoint, STPoint>>& point_pairs) const {
  if (point_pairs.size() < 2) {
    AERROR << "point_pairs.size() must >= 2, but current point_pairs.size() = " << point_pairs.size();
    return false;
  }
  ...
  for (size_t i = 0; i < point_pairs.size(); ++i) {
    ...
    if (i + 1 != point_pairs.size()) {
      ...
      if (std::fmax(curr_lower.t(), curr_upper.t()) + kMinDeltaT >= std::fmin(next_lower.t(), next_upper.t())) {
        AERROR << "Latter points should have larger t: " << "curr_lower[" << curr_lower.DebugString() << "] curr_upper["
        << curr_upper.DebugString() << "] next_lower[" << next_lower.DebugString() << "] next_upper["
