...
bool STBoundaryMapper::MapStopDecision(Obstacle* stop_obstacle, const ObjectDecisionType& stop_decision) const {
  ...
  if (stop_ref_s > path_data_.frenet_frame_path().back().s()) {
    st_stop_s = path_data_.discretized_path().back().s() + (stop_ref_s - path_data_.frenet_frame_path().back().s());
  } else {
...
const Obstacle& obstacle, std::vector<STPoint>* upper_points, std::vector<STPoint>* lower_points) const {
  // Sanity checks.
  ...
  const auto* planning_status = injector_->planning_context()->mutable_planning_status()
  ->mutable_change_lane();

  double l_buffer = planning_status->status() == ChangeLaneStatus::IN_CHANGE_LANE
  ? FLAGS_lane_change_obstacle_nudge_l_buffer : FLAGS_nonstatic_obstacle_nudge_l_buffer;
  ...
  if (trajectory.trajectory_point().empty()) {
    ...
    //2、针对预测轨迹上的每个点，对ADC PATH上的点进行遍历，找到存在overlap的点；
    //3、将所有重合点的时间t以及自车路径的位置s进行记录；
    for (int i = 0; i < trajectory.trajectory_point_size(); ++i) {
      ...
      for (double path_s = 0.0; path_s < path_len; path_s += step_length) {
        ...
        /*接下来针对目标的预测轨迹上的每一个点，都对路径上的每一个点进行遍历，重点是寻找的重合的点,
        并根据buffer值精细找到当前时刻与自车路径相交的上下边界*/
...
void STBoundaryMapper::ComputeSTBoundaryWithDecision(Obstacle* obstacle, const ObjectDecisionType& decision) const {
  ...
  if (decision.has_follow()) {
    ...
    boundary = STBoundary::CreateInstance(lower_points, upper_points).ExpandByS(characteristic_length);
...
bool STBoundaryMapper::CheckOverlap(const PathPoint& path_point, const Box2d& obs_box,
const double l_buffer) const {
  ...
  vehicle_param_.back_edge_to_center()) * 0.5, (vehicle_param_.left_edge_to_center() -
