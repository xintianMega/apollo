...
PathDecider::PathDecider(const TaskConfig &config, const std::shared_ptr<DependencyInjector> &injector)
...
PathDecision *const path_decision) {
  ...
  for (const auto *obstacle : path_decision->obstacles().Items()) {
    ...
    //如果障碍物挡住了路径，加stop决策,根据 blocking_obstacle_id 判断是否挡住路径，这个变量在前面的task中会进行计算
    ...
    //nudge判断，如果距离静态障碍物距离太远，则忽略。如果静态障碍物距离车道中心太近，则停止。如果横向方向很近，则避开。
    ...
    if (curr_l - lateral_radius > sl_boundary.end_l() || curr_l + lateral_radius < sl_boundary.start_l()) {
      ...
      *object_decision.mutable_stop() = GenerateObjectStopDecision(*obstacle);
      ...
      reference_line_info_->reference_line(), reference_line_info_->AdcSlBoundary())) {
