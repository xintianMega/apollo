...
const ReferenceLine& reference_line, const common::TrajectoryPoint& init_point, const bool path_reusable,
...
ADEBUG << "Plan at the starting point: x = " << init_point.path_point().x() << ", y = "
<< init_point.path_point().y() << ", and angle = " << init_point.path_point().theta();
...
for (const auto& path_boundary : path_boundaries) {
  ...
  reference_path_data.is_valid_path_reference()) {
    ...
    for (size_t i = 0; i < path_reference_size; ++i) {
      ...
      reference_path_data.path_reference().at(i).x(), reference_path_data.path_reference().at(i).y()),
      &path_reference_sl);
  ...
  const double lat_acc_bound = std::tan(veh_param.max_steer_angle() / veh_param.steer_ratio()) / veh_param.wheel_base();
  ...
  bool res_opt = OptimizePath(init_frenet_state, end_state, std::move(path_reference_l),
  ...
  if (res_opt) {
    ...
    auto frenet_frame_path = ToPiecewiseJerkPath(opt_l, opt_dl, opt_ddl, path_boundary.delta_s(), path_boundary.start_s());
...
if (end_state[0] != 0 && !is_valid_path_reference) {
  ...
  const auto& pull_over_type = injector_->planning_context()->planning_status().pull_over().pull_over_type();
  const double weight_x_ref = pull_over_type == PullOverStatus::EMERGENCY_PULL_OVER ? 200.0 : 10.0;
...
if (is_valid_path_reference) {
  ...
  const double peak_value = config_.piecewise_jerk_path_optimizer_config() .path_reference_l_weight();
...
piecewise_jerk_problem.set_dx_bounds(-FLAGS_lateral_derivative_bound_default, FLAGS_lateral_derivative_bound_default);
...
const double jerk_bound = EstimateJerkBoundary(std::fmax(init_state.first[1], 1.0), axis_distance, max_yaw_rate);
...
if (!success) {
  ...
  AERROR << "dl bound" << FLAGS_lateral_derivative_bound_defaul << " jerk bound" << jerk_bound;
  ...
  AERROR << "lat boundary, ddl boundary , path reference" << std::endl << ssm.str();
  return false;
}
